...
class Transactions extends Component
{
    ...
    public function chart(): void
    {
        $query = Sale::when(
            ...
            static fn ($q) => $q->selectRaw("SUM(total_amount) as total, SUM(due_amount) as due_amount, strftime('%m', date) as labels, COUNT(*) as sales")
                ->whereRaw("strftime('%Y', date) = ?", [date('Y')])
                ->groupByRaw("strftime('%m', date)"),
            static fn ($q) => $q->selectRaw("SUM(total_amount) as total, SUM(due_amount) as due_amount, strftime('%Y', date) as labels, COUNT(*) as sales")
        ...
        $query = Purchase::when(
            $this->typeChart === 'monthly',
            static fn ($q) => $q->selectRaw("SUM(total_amount) as total, SUM(due_amount) as due_amount, strftime('%m', date) as labels, COUNT(*) as purchases")
                ->whereRaw("strftime('%Y', date) = ?", [date('Y')])
                ->groupByRaw("strftime('%m', date)"),
            static fn ($q) => $q->selectRaw("SUM(total_amount) as total, SUM(due_amount) as due_amount, strftime('%Y', date) as labels, COUNT(*) as purchases")
                ->groupByRaw("strftime('%Y', date)")
        )->get()->toArray();
    ...
    public function monthlyChartOptions(): array
    {
        ...
        ->groupBy(DB::raw("strftime('%m', sale_payments.created_at)"))  // Group by month
        ...
        ->get();
            ...
            ->groupBy(DB::raw("strftime('%m', purchase_payments.created_at)"))  // Group by month
            ->select(DB::raw('SUM(purchase_payments.amount) / 100 as purchase_total_payment'), 'suppliers.name as supplier_name')
            ->get();
    ...
    public function paymentChart(): array
    {
        ...
        ->groupBy(DB::raw("strftime('%Y-%m', date)"))  // Group by formatted date
        ->orderBy('month')
        ->get();
